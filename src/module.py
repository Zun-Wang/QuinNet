...
class LNNP(LightningModule):
    def __init__(self, hparams, prior_model=None, mean=None, std=None):
        ...
        self.save_hyperparameters(hparams, ignore=['prior_model'])
    ...
    def forward(self, z, pos, nv=None, batch=None, q=None, s=None):
        return self.model(z, pos, nv=nv, batch=batch, q=q, s=s)
    ...
    def training_epoch_end(self, training_step_outputs):
        ...
        if hasattr(dm, 'test_dataset') and len(dm.test_dataset) > 0:
            should_reset = (self.current_epoch + 2) % self.hparams.test_interval == 0 or (self.current_epoch + 1) % self.hparams.test_interval == 0
    ...
    def test_epoch_end(self, outputs) -> None:
        ...
        self.log_dict(result_dict, sync_dist=True, logger=True)
        self._reset_losses_dict()
